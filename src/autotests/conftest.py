...
def pytest_generate_tests(metafunc):
    """функция для отладки теста, чтобы запустить его несколько раз подряд
    пример запуска: pytest.exe src\autotests\tests\test_moscow_tech_school.py --repeat 10
    в примере тест запустится 10 раз подряд"""
